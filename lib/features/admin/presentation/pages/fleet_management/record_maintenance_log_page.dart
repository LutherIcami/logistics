...
class RecordMaintenanceLogPage extends StatefulWidget {
  const RecordMaintenanceLogPage({super.key});

  @override
  State<RecordMaintenanceLogPage> createState() =>
      _RecordMaintenanceLogPageState();
}

class _RecordMaintenanceLogPageState extends State<RecordMaintenanceLogPage> {
  ...
  Future<void> _handleSubmit() async {
    ...
    final log = MaintenanceLog(
      ...
      serviceProvider: _providerController.text.trim(),
      nextServiceOdometer: _nextOdometerController.text.isNotEmpty
          ? double.parse(_nextOdometerController.text)
          : null,
    ...
    if (mounted) {
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Maintenance log recorded successfully'),
          ),
        );
        context.pop();
      } else {
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildSectionTitle('NEXT SCHEDULED SERVICE'),
                  Row(
                    children: [
                      Expanded(
                        child: _buildTextField(
                          controller: _nextOdometerController,
                          label: 'Next Odometer (km)',
                          icon: Icons.upcoming,
                          keyboardType: TextInputType.number,
                        ),
                      ),
  ...
  Widget _buildSubmitButton() {
    return SizedBox(
      ...
      child: FilledButton.icon(
        ...
        label: Text(_isSubmitting ? 'RECORDING...' : 'RECORD SERVICE'),
